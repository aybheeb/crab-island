...
export async function getItemBreakdown({ from, to }) {
  const { rows } = await query(
    `select oi.item_name as name, oi.category, sum(oi.qty)::int as qty, sum(oi.line_total) as revenue
     from order_items oi
  ...
  return rows.map((r) => ({ name: r.name, category: r.category, qty: r.qty, revenue: Number(r.revenue) }));
}
